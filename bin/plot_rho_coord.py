...
def to_show_or_not_to_show(savefig: bool, displayfig: bool, name: str) -> None:
    if displayfig:
        if not savefig:
            plt.show()
        else:   # savefig == True
...
def main(argv = None):
    ...
    headers = ""
    scale = 1e3 # Expecting material thickness to be in mm
    mass = []
    theta0 = []
    ...
    for file in datafiles:
        ...
        # header is: distance, pnn, pmn, pnm, pmm
        # indices are:  0       1    2    3    4
        ax.semilogy(data[:,0] * scale, data[:,1], label = r"$\rho_{{nn}}  $; m = {}, $\theta_0$ = {}".format(mass[-1], theta0[-1]), marker = '.', ls = "dotted")
        #ax.semilogy(data[:,0] * scale, data[:,2], label = r"$\rho_{{nn'}}$; m = {}, $\theta_0$ = {}".format(mass[-1], theta0[-1]), marker = '.', ls = "dotted")
        ax.semilogy(data[:,0] * scale, data[:,4], label = r"$\rho_{{n'n'}}$; m = {}, $\theta_0$ = {}".format(mass[-1], theta0[-1]), marker = '.', ls = "dotted")
